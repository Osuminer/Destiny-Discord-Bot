...
class Manifest:
    ...
    def decode_hash(self, hash_id, definition, language):
        ...
        with DBase("world_sql_content_4c97503460963fcd2ce43e97c372be87.content") as db:
            try:
                res = db.query(hash_id, definition, identifier)
            except sqlite3.OperationalError as e:
                if e.args[0].startswith('no such table'):
                    raise PyDest.PydestException(
                        "Invalid definition: {}".format(definition))
                else:
                    raise e
    ...
    async def update_manifest(self, language):
        ...
        if json['ErrorCode'] != 1:
            raise PyDest.PydestException(
                "Could not retrieve Manifest from Bungie.net")

        manifest_url = 'https://www.bungie.net' + \
            json['Response']['mobileWorldContentPaths'][language]
